...
source "$HOME/aetheron/scripts/actions-lib.sh"
...
log() { echo "[$(date '+%Y-%m-%d %H:%M:%S')] $1" | tee -a "$ORCH_LOG"; }
summary_append() { echo "[$(date '+%Y-%m-%d %H:%M:%S')] $1" | tee -a "$SUMMARY_LOG"; }
show_status() {
...
preflight_checks() {
  ...
  # Internet (DNS + HTTPS)
  if ! getent hosts archlinux.org >/devnull; then
  ...
  if command -v timedatectl >/dev/null 2>&1; then
    if ! timedatectl show -p NTPSynchronized --value 2>/dev/null | grep -q true; then
  ...
  check_space "/srv"  2048   # 2GB (falls genutzt)
...
init_order_file_if_missing() {
  if [[ ! -f "$ORDER_FILE" ]]; then
    ...
    find "$SERVICES_DIR" -maxdepth 1 -mindepth 1 -type d -printf "%f\n" \
      | grep -viE 'templates|^\.|^cert|lets' | sort > "$ORDER_FILE"
    log "Created $ORDER_FILE"
  fi
}

read_services_order() {
  mapfile -t SERVICES_ORDER < <(grep -v '^\s*$' "$ORDER_FILE" | sed 's/\r$//')
}
...
autofix_order_or_abort() {
  ...
  local changed=false
  if (( ${#missing[@]} > 0 )) || (( ${#added[@]} > 0 )); then changed=true; fi
...
script_path_for() {
  local service="$1"; local action="$2"
  echo "$SERVICES_DIR/$service/${action}.sh"
}

# --- DRY-RUN Binder (für Libs) ---
_bind_dryrun() {
  if [[ "${DRYRUN:-false}" == "true" ]]; then export AETHERON_DRYRUN=1; else unset AETHERON_DRYRUN; fi
}

# --- Wipe glue (optional direkt nutzbar) ---
...
do_action_one() {
  ...
  # Wipe läuft zentral über wipe-lib (Configs/Compose/Secrets sichern+löschen; Daten bleiben)
  if [[ "$action" == "wipe" ]]; then
    log "RUN: $service – wipe"
    if wipe_service "$service"; then
      summary_append "OK: $service (wipe)"
      progress_bar
      return 0
    else
      summary_append "FAIL: $service (wipe)"
      return 1
    fi
  fi

  # uninstall: service/uninstall.sh bevorzugt, sonst Fallback (actions-lib)
  if [[ "$action" == "uninstall" ]]; then
    local script; script="$(script_path_for "$service" uninstall)"
    if [[ -x "$script" ]]; then
      log "RUN: $service – uninstall.sh"
      if [[ "$DRYRUN" == true ]]; then
        summary_append "DRY-RUN: $service (uninstall)"; progress_bar; return 0
      fi
      ...
      if fallback_uninstall_service "$service"; then
  ...
  # reinstall: service/reinstall.sh bevorzugt, sonst Fallback (down + install.sh)
  if [[ "$action" == "reinstall" ]]; then
    local script; script="$(script_path_for "$service" reinstall)"
    if [[ -x "$script" ]]; then
      log "RUN: $service – reinstall.sh"
      if [[ "$DRYRUN" == true ]]; then
        summary_append "DRY-RUN: $service (reinstall)"; progress_bar; return 0
      fi
      ...
      if fallback_reinstall_service "$service"; then
  ...
  log "SKIP: $service – unknown action '$action'"
  return 2
}
...
do_action_many() {
  ...
  echo "(Detail: $SUMMARY_LOG)"; echo; read -rp "Weiter mit [Enter] ..." _
}
...
ensure_templates() {
  ...
  if [[ ! -d "$TEMPLATES_DIR" ]]; then mkdir -p "$TEMPLATES_DIR"; fi
...
add_service() {
  ...
  for f in install.sh reinstall.sh uninstall.sh wipe.sh; do
    ...
    sed -i "s/^# Version: .*/# Version: 1.00.00/" "$SERVICES_DIR/$name/$f"
...
manage_services_menu() {
  while true; do
    ...
    case "$c" in
      ...
      3) rename_service ;;
      4) reorder_services ;;
      5) echo; nl -ba "$ORDER_FILE"; echo; read -rp "Weiter [Enter]..." _ ;;
...
main_menu() {
  ...
  while true; do
    ...
    case "$choice" in
      ...
      6) [[ -f "$SUMMARY_LOG" ]] && { echo; cat "$SUMMARY_LOG"; echo; read -rp "Weiter [Enter]..." _; } || { echo "No summary yet."; sleep 1; } ;;
      7) manage_services_menu ;;
      8) echo; nl -ba "$ORDER_FILE"; echo; read -rp "Weiter [Enter]..." _ ;;
      9) echo "Bye."; exit 0 ;;
      *) echo "Invalid option"; sleep 1 ;;
    esac
  done
